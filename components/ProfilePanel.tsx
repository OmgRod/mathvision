      ...
      >
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-20">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-900">Achievements Hall</h2>
              <p className="text-slate-500 text-sm font-medium">Your collection of mathematical milestones</p>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className={`p-4 rounded-2xl ${unlocked ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-slate-200 text-slate-400'}`}>
                      {unlocked ? (ICON_MAP[ach.icon] || <Award size={24} />) : <Lock size={24} />}
                    </div>
                    {unlocked ? (
                      <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase tracking-widest animate-pulse">
                        Unlocked
                      </div>
                    ) : (
                  ...
                  <div className="space-y-1">
                    <h4 className="font-black text-slate-900 text-lg leading-tight">{ach.title}</h4>
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
             ...
             <div>
               <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-none">STUDENT PROFILE</h1>
               <p className="text-slate-500 font-bold mt-1 uppercase tracking-widest text-xs">Level {profile.level} Master Academic</p>
             </div>
          </div>
        </div>

        <div className="flex gap-4">
          <label className="flex items-center gap-2 px-6 py-4 bg-white border border-slate-200 rounded-2xl font-black text-slate-600 hover:bg-slate-50 transition-all cursor-pointer">
            <Upload size={20} />
            IMPORT DATA
          ...
          <button 
            ...
            className="flex items-center gap-2 px-6 py-4 bg-slate-900 text-white rounded-2xl font-black hover:bg-slate-800 transition-all shadow-xl shadow-slate-200"
          >
            <Download size={20} />
            EXPORT ALL DATA
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="relative z-10 space-y-10">
              <div className="flex justify-between items-end">
                <div className="space-y-2">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Global Experience</h3>
                  <div className="text-7xl font-black text-slate-900 leading-none">{profile.xp} <span className="text-indigo-600 text-3xl">XP</span></div>
                </div>
                <div className="text-right">
                  <span className="text-xs font-black text-indigo-600 uppercase tracking-widest">Next Level In</span>
                  <div className="text-3xl font-black text-slate-900">{xpToNext} XP</div>
                </div>
              </div>
        ...
        <div className="space-y-8">
           <div className="bg-slate-50 p-8 rounded-[3rem] border border-slate-100 space-y-8">
              ...
              <button 
                onClick={() => setShowAchievementsModal(true)}
                className="w-full py-4 bg-white border border-indigo-100 text-indigo-600 rounded-2xl font-black text-xs hover:bg-indigo-50 transition-all flex items-center justify-center gap-2"
              >
                VIEW FULL HALL <ChevronRight size={14} />
              </button>
           </div>

           <div className="p-8 bg-indigo-600 rounded-[3rem] text-white space-y-6 shadow-2xl relative overflow-hidden">
...
const StatCard: React.FC<{ icon: React.ReactNode, label: string, value: number | string, color: string }> = ({ icon, label, value, color }) => (
  <div className={`p-8 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-6 ${color}`}>
...
const AchievementItem: React.FC<{ icon: React.ReactNode, title: string, desc: string, unlocked?: boolean, progress?: number }> = ({ icon, title, desc, unlocked, progress }) => (
  <div className={`p-4 rounded-2xl transition-all ${unlocked ? 'bg-white shadow-sm' : 'bg-white/50'}`}>
    <div className="flex items-start gap-4">
      <div className={`p-2 rounded-lg shrink-0 ${unlocked ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-400'}`}>
        {icon}
      </div>
      <div className="flex-grow">
        <div className="font-black text-slate-900 text-sm">{title}</div>
        <div className="text-[10px] font-bold text-slate-500 uppercase tracking-tight line-clamp-1">{desc}</div>
