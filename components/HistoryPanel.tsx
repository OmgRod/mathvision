...
export const HistoryPanel: React.FC<HistoryPanelProps> = ({ onSelectItem }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-40 space-y-6 text-center px-4">
        ...
        <div className="max-w-xs">
          <h3 className="text-xl font-black text-slate-900 mb-2">Your history is empty</h3>
          <p className="text-slate-500 font-medium italic">"Every master was once a beginner. Start solving or learning to see your journey here!"</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 pb-40">
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-12 gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm md:text-base text-slate-500 font-medium">Revisit your past solutions and lessons</p>
        </div>
        <button
          onClick={() => setModalConfig({ isOpen: true, type: 'clear' })}
          className="flex items-center justify-center gap-2 px-4 py-2 text-rose-500 hover:bg-rose-50 rounded-xl transition-all font-bold text-sm w-fit"
        >
          <Trash2 size={16} />
          Clear All
        </button>
        ...
        <AnimatePresence mode="popLayout">
          {history.map((item, idx) => (
            <motion.div
              ...
              className="group bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:border-indigo-200 transition-all flex items-center gap-6 relative overflow-hidden"
            >
              <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-50 rounded-2xl flex items-center justify-center group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors shrink-0">
                {getTypeIcon(item.type)}
              </div>

              <div className="flex-grow min-w-0 pointer-events-none">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-indigo-400">
                    {getTypeLabel(item.type)}
                  </span>
                  <span className="text-[10px] text-slate-300">•</span>
                  <span className="text-[8px] md:text-[10px] font-bold text-slate-400 flex items-center gap-1">
                    <Calendar size={10} />
                    {formatDate(item.timestamp)}
                  </span>
                </div>
                <h4 className="text-base md:text-lg font-black text-slate-900 truncate group-hover:text-indigo-600 transition-colors">
      ...
      <div className="mt-12 p-8 bg-indigo-50 rounded-[2.5rem] border border-indigo-100 flex gap-4">
